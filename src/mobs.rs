...
fn spawn_snakes(
    ...
    if query.iter().count() < 2 { // Reduced from 3 to 2 snakes to improve performance
        ...
        if voxel_world.chunks.contains_key(&chunk_coord) && !voxel_world.blocks.contains_key(&pos) {
             ...
             // Initialize history
             for i in 0..10 { // Reduced from 20 to 10 for performance
                 history.push(pos.as_vec3() + Vec3::new(0.0, -0.25, 0.0) + Vec3::Y * (i as f32 * 0.2));
             }

             // Spawn initial segments - reduced from 15 to 8 for performance
             for i in 0..8 {
...
fn sand_snake_ai(
    ...
    time: Res<Time>,
    mut snake_update_timer: Local<Option<Timer>>,
) {
    // Initialize the timer if it hasn't been initialized yet
    if snake_update_timer.is_none() {
        *snake_update_timer = Some(Timer::from_seconds(0.2, TimerMode::Repeating)); // Slower update rate
    }

    let timer = snake_update_timer.as_mut().unwrap();

    // Only update snake AI periodically to reduce performance impact
    ...
    for (entity, mut snake) in snake_query.iter_mut() {
        ...
        // Check if the snake entity still exists in the world before proceeding
        let pos = {
            if let Ok(transform) = transform_query.get(entity) {
                transform.translation.round().as_ivec3()
            } else {
                continue; // Skip if the entity doesn't exist anymore
            }
        };
            ...
            if let Some(&target_entity) = voxel_world.blocks.get(&target) {
                if let Ok(block_type) = block_type_query.get(target_entity) {
                    if block_type.0 == 7 { // Sand
                        ...
                        // Reduce particles spawned for performance
                        if let Ok(mat_handle) = block_material_query.get(target_entity) {
                            for i in 0..2 { // Reduced from 5 to 2 particles
        ...
        if let Some(target) = target_opt {
            // Get the transform again to update the position
            if let Ok(mut transform) = transform_query.get_mut(entity) {
                ...
                // Update Segments Visuals - limit updates for performance
                let spacing = 0.2;
                let mut prev_pos = snake.history[0];

                // Only update a subset of segments to improve performance
                let num_segments_to_update = (snake.segments.len() / 2).max(1); // Update only half the segments

                for (i, &seg_entity) in snake.segments.iter().enumerate().take(num_segments_to_update) {
                ...
                if growing {
                    ...
                    // Limit the number of segments to prevent performance degradation
                    if snake.segments.len() < 20 { // Maximum 20 segments
